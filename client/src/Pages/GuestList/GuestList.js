...
class GuestList extends Component {
  ...
  createGuest = (e) => {
    e.preventDefault();
    API.createNewGuest({ firstName: this.state.firstName, lastName: this.state.lastName, status: this.state.status, peopleCount: this.state.peopleCount })
      .then(({ data }) => {
        this.setState({ Rsvp: data });
        // console.log(this.state);
      })
  ...
  createEvent = (e) => {
    e.preventDefault();
    API.createNewEvent({ eventName: this.state.eventName, locationName: this.state.locationName, date: this.state.date })
      .then(({ data }) => {
        this.setState({ Events: data });
        // console.log(this.state);
      })
  ...
  render() {
    ...
    return (
      <div>
        ...
        <span>
          <button className="cursor-style" onClick={() => this.guests("guest")}>Guest List</button>
        </span>
        <span>
        ...
        {this.state.showEvents ?
          ...
          <div>
            ...
            {(this.state.showAdd == true) ? (<button className="row-style p-0 mx-5 my-1" onClick={() => this.eventForm("clicked")}>+</button>) : null}
            {(this.state.showEventForm == true) ? (<form className="row-style p-0 mx-5 my-1">
            <input
                type="text"
                name="eventName"
          ...
          <div>
            ...
            {(this.state.showAdd == true) ? (<button className="row-style p-0 mx-5 my-1" onClick={() => this.guestForm("clicked")}>+</button>) : null}
            {(this.state.showGuestForm == true) ? (<form className="row-style p-0 mx-5 my-1">
              <input
                type="text"
                name="firstName"
              ...
              <button type="submit" onClick={this.createGuest}>create</button>
            </form>) : null}
          </div>
        }
      </div>
